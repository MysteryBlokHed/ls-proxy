...
const throwIfError = (valid, defaultError) => {
...
const defaultStoreSeparateConfig = ({ id, checkGets, set, get, validate, modify, parse, stringify, }) => {
    return Object.assign({ id, validate: validate !== null && validate !== void 0 ? validate : (() => true), modify: modify !== null && modify !== void 0 ? modify : (value => value) }, commonDefaults({ checkGets, set, get, parse, stringify }));
};
/**
...
function storeSeparate(defaults, configuration = {}) {
    ...
    /** Call validOrThrow with relevant parameters by default */
    const vot = (value, key, action = 'set') => validOrThrowSeparate(validate, modify, value, action, key);
    /** Parse a value from localStorage and set it on the object if valid */
    const checkSetFromLs = (value, key) => {
    ...
    const checkStringify = (key) => stringify(vot(object, key)[key]);
    // Set defaults
    for (const key of Object.keys(defaults)) {
        const keyPrefix = addId(key, id);
        const lsValue = get(keyPrefix);
        if (!lsValue)
            set(keyPrefix, checkStringify(key));
        else
            checkSetFromLs(lsValue, key);
    }
